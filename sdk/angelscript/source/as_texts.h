...
#define TXT_CANNOT_RESOLVE_AUTO_AMBIGUOUS          "Unable to resolve auto type, the expression is an ambiguous name"
#define TXT_CANNOT_RESOLVE_AUTO_LAMBDA             "Unable to resolve auto type, the expression is an anonymous function"
#define TXT_CANNOT_RESOLVE_AUTO_NULL               "Unable to resolve auto type, the expression is a null handle"
...
#define TXT_FAILED_TO_CREATE_STRING_CONSTANT      "Failed to create string constant"
...
#define TXT_INVALID_DECLARATION_s                  "Invalid declaration: '%s'"
...
#define TXT_NO_MATCHING_SYMBOL_s_MASK_x_vs_x       "No matching symbol '%s' (hidden by access mask: candidate mask 0x%X vs module mask 0x%X)"
...
#define TXT_s_NOT_MEMBER_OF_s_MASK_x_vs_x          "'%s' is not a member of '%s' (hidden by access mask: member mask 0x%X vs module mask 0x%X)"
...
#define TXT_TEMPLATE_CTOR_FIRST_PARAM_MUST_BE_SUBTYPE    "First parameter to a template type constructor must be the hidden subtype reference (e.g. int&in), not the object type itself. A copy constructor must be declared as 'void f(int&in, const T<U> &in)'"
...
#define TXT_BYTECODE_NO_FORMAT_HEADER                    "LoadByteCode failed: the stream has no format header (saved by an older library version). Rebuild the bytecode from source"
#define TXT_BYTECODE_FORMAT_VERSION_d                    "LoadByteCode failed: bytecode format version %u does not match this library. Rebuild the bytecode from source"
